...
{
    public partial class ucListaDispositivos_gv : DevExpress.XtraEditors.XtraUserControl
    {
        public ucListaDispositivos_gv()
        {
            InitializeComponent();
            var menu = new common.MenuItemGridViews(ref gvDatos, "Lista de Dispositivo");
        }
        ...
        {
            var idServidor = gvDatos.GetFocusedRowCellValue(colId);
            if (Utilerias.IsNullOrEmpty(idServidor))
                return;

            var disp = gvDatos.GetFocusedRowCellValue(colDescripcion);
        ...
        {
            var idServidor = Utilerias.SafeToString(gvDatos.GetFocusedRowCellValue(colId));
            if (Utilerias.IsNullOrEmpty(idServidor))
                return;
